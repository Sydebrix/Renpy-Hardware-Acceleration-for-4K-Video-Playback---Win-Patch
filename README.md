...
- Download the files by doing one of these:
  - Click on the "Code" button above on this github page and download the repository as a zip file.
  - Select the latest release on the right and download the zip-file. 
- Extract it
...
- Supports over-sampling behavior equivalent to Renpy's native one, e.g. `video@2.webm` will be handled as a video with double the size in x and y and played correctly.

## Caveat
- Works only on Windows.
- Volume isn't taken from Renpy and has to be set manually. It is 50% by default, and can be changed directly in the `mediaexports.py` on line 50:
```python
